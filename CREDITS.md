...
42 изображения лицевых сторон и изображение общей оборотной стороны предоставлены пользователем проекта и скопированы в `assets/cards/` без художественного редактирования. Карточки 16 и 18 заменены изображениями из «Загрузок», карточка 42 («Лягушка Оли») добавлена оттуда же. Исходники в `Визуальный стиль/карточки` остаются нетронутыми. Внешний светлый фон у лицевых изображений маскируется игровым шейдером `scripts/visual/card_surface.gdshader`; отдельный файл `scripts/ui/card_preview.gdshader` применяет ту же маску к превью коллекции.
...
Для nappin и RRFreelance пользователь явно разрешил продолжить работу с бесплатными файлами для локального проекта, без дальнейшей проверки лицензии. Это не является подтверждением лицензии на распространение. Никаких лицензий этим моделям от имени авторов не присвоено.
